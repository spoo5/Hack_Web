...
Executes queries using DuckDB (preferred) or Pandas as fallback.
ALL COMPUTATIONS ARE DETERMINISTIC - NO HALLUCINATION.
"""
...
# DuckDB engine for SQL execution (optional path used when LLM provides SQL)
try:
...
class AnalyticsEngine:
    """Executes data analytics operations using DuckDB or Pandas"""
    ...
    def execute(
        ...
        """
        Execute parsed query on DataFrame (or DuckDB connection).

        If the parsed_query contains 'llm_sql' (generated by the LLM) and a
        DuckDB connection is provided, execution is delegated to DuckDB.
        Otherwise, falls back to pure-Pandas computation.

        Args:
            df: Source DataFrame
            parsed_query: Parsed query structure
            con: Optional DuckDB connection (used for LLM-generated SQL)
            
        Returns:
            Result dictionary with computed values
        """
        llm_sql = parsed_query.get("llm_sql")
        if llm_sql and con is not None and _duckdb_available:
            return self._execute_duckdb(llm_sql, con, parsed_query)
    ...
    def _execute_duckdb(
        ...
        """Execute a validated SQL query via DuckDB and return a result dict."""
        try:
            query_result = run_query(sql, con)
            if query_result.error:
                return {"type": "error", "message": query_result.error}
